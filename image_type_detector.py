...
def _stats(image_bytes: bytes) -> dict[str, float]:
    ...
    gray = arr.mean(2)
    # گرادیان‌های ساده به‌جای sobel؛ برای سنجش لبه و وضوح کافی است
...
def _classify(s: dict[str, float]) -> tuple[str, float, str]:
    ...
    # ۱) نوار قلب: زمینه‌ی سفید گسترده + نسبت دراز + لبه‌ی نازک
    ...
    # ۲) برگه‌ی آزمایش/نسخه: زمینه‌ی سفید + متن (لبه‌های متراکم) + نسبت عادی صفحه
    ...
    # ۳) رادیولوژی: تقریباً بی‌رنگ و بدون زمینه‌ی سفید (تیره‌رو یا خاکستریِ یکنواخت)
    ...
    # ۴) زخم/سوختگی: قرمزی خیلی شدید، یا قرمزی بالا همراه با نواحی تیره
    ...
    # ۶) عکس چشم: سفیدی اطراف + قرینی/کروی — قابل تفکیک مطمئن نیست؛ حدس ضعیف
